...
def render_settings(config, alpaca=None, stellarium=None):
    ...
    with st.form("settings_form"):
        ...
        st.caption(
            "Configure the network connection to your Seestar S50 telescope. "
        ...
        st.caption(
            "Connect to Stellarium's Remote Control plugin for visual target "
            "selection. Enable it in Stellarium: Configuration > Plugins > Remote Control."
        )
        st_host = st.text_input(
            "Stellarium Host",
        ...
        st.caption(
            "Default values used when the Imaging page loads for the first time. "
            "These can be changed per-session on the Imaging page."
        )
        default_gain = st.number_input(
            "Default Gain",
    ...
    with col1:
        if st.button(
            "Test ALPACA",
            help="Send a test request to the ALPACA server "
    ...
    with col2:
        if st.button(
            "Test Stellarium",
            help="Check if Stellarium's Remote Control "
    ...
    if st.button(
        "Fetch Device Info",
        help="Reads identification info from all 5 "
        ...
        for device in devices:
            try:
                ...
                name = (
                    name_resp.json().get("Value", "N/A") if name_resp.status_code == 200 else "N/A"
                )
                desc = (
                    desc_resp.json().get("Value", "N/A") if desc_resp.status_code == 200 else "N/A"
                )
