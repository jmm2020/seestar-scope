...
def render_config_panel(is_running: bool):
    ...
    with st.expander("⚙️ Configuration", expanded=not is_running):
        ...
        st.caption(
            f"**Sweep range:** ±{total_range // 2} steps from current position ({total_range} total)"
        )
...
def render_control_buttons(is_running: bool):
    ...
    with col1:
        if st.button(
            "▶️ Start Autofocus", disabled=is_running, use_container_width=True, type="primary"
        ):
            config = st.session_state.autofocus_config
...
def start_autofocus(config: Dict[str, Any]):
    ...
    try:
        with st.spinner("Starting autofocus routine..."):
            response = requests.post(f"{BACKEND_URL}/api/autofocus/start", json=config, timeout=10)
...
def abort_autofocus():
    ...
    try:
        with st.spinner("Aborting autofocus..."):
            response = requests.post(f"{BACKEND_URL}/api/autofocus/abort", timeout=5)
