...
def render_solve_controls():
    ...
    mode = st.radio(
        "Solving Mode",
        ["hint", "blind"],
        format_func=lambda x: "🎯 Hint Mode (with position)" if x == "hint" else "🌌 Blind Mode (full-sky search)",
    ...
    if solve_button:
        if uploaded_file:
            execute_solve_upload(uploaded_file, mode, expected_ra_hours if mode == "hint" else None,
                                expected_dec_degrees if mode == "hint" else None,
                                search_radius_deg if mode == "hint" else None,
                                fov_deg, downsample)
        else:
            execute_solve_path(image_path, mode, expected_ra_hours if mode == "hint" else None,
                              expected_dec_degrees if mode == "hint" else None,
                              search_radius_deg if mode == "hint" else None,
                              fov_deg, downsample)
    ...
    with st.spinner("🔄 Solving..."):
        try:
            response = requests.post(
                f"{BACKEND_URL}/api/platesolve/solve",
                json=request_data,
                timeout=180
            )
        ...
        if solution:
            ...
            if result.get('mode') == 'hint' and result.get('offset_arcsec') is not None:
                ...
                with col_offset2:
                    st.metric(
                        "RA Offset",
                        f"{result.get('offset_ra_arcsec', 0):.1f}″"
                    )
                
                with col_offset3:
                    st.metric(
                        "Dec Offset",
                        f"{result.get('offset_dec_arcsec', 0):.1f}″"
                    )
                ...
                if offset_arcsec < 30:
                    ...
                    st.error("❌ Large offset detected. Telescope may need polar alignment or calibration.")
...
def render_session_card(session_id: str):
    ...
    try:
        response = requests.get(
            f"{BACKEND_URL}/api/platesolve/result/{session_id}",
            timeout=5
        )
            ...
            with st.expander(f"{status_icon} {session_id[:12]}... • {mode_badge} • {format_timestamp(timestamp)}"):
                ...
                if status == "success" and result.get('solution'):
                    ...
                    with col2:
                        ...
                        st.markdown(f"**FOV:** {solution['fov_width']:.3f}° × {solution['fov_height']:.3f}°")
                        st.markdown(f"**Stars:** {solution['num_stars']}")
                    
                    # Offset (hint mode)
                    if mode == "hint" and result.get('offset_arcsec') is not None:
                        st.divider()
                        st.markdown(f"**Offset:** {result['offset_arcsec']:.1f}″ (RA: {result.get('offset_ra_arcsec', 0):.1f}″, Dec: {result.get('offset_dec_arcsec', 0):.1f}″)")
