...
def render_gallery():
    ...
    if not check_backend_health():
        st.error("Backend API is not reachable at localhost:8503. Start the FastAPI backend first.")
        st.code("cd backend && uvicorn backend.main:app --host 0.0.0.0 --port 8503", language="bash")
        return
...
def render_image_card(image: Dict[str, Any]):
    ...
    with st.container():
        ...
        st.caption(f"{metadata['exposure']}s @ Gain {metadata['gain']}")
