...
def save_dual_format(
    ...
    fits_path = save_fits(
        image_data_raw, metadata, target_name, save_dir=str(Path(save_dir) / "fits")
    )

    # Save PNG (preview)
    png_path = save_image(image_pil, target_name, save_dir=str(Path(save_dir) / "png"))
...
def prepare_metadata_from_alpaca(
    alpaca_client, target_name: str, exposure: float, gain: int
) -> Dict[str, Any]:
