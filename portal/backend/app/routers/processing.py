...
class ProcessSessionRequest(BaseModel):
    ...
    metadata: Optional[Dict] = Field(
        default=None, description="Optional metadata (exposure, gain, etc.)"
    )
...
async def process_session(request: ProcessSessionRequest, background_tasks: BackgroundTasks):
    ...
    **Example:**
    ...
    try:
        ...
        if missing:
            raise HTTPException(status_code=400, detail=f"FITS files not found: {missing}")
        ...
        if request.session_id in processing_tasks:
            ...
            if existing.status in [
                ...
                ProcessingStatus.STACKING,
            ]:
                raise HTTPException(
                    status_code=409, detail=f"Session {request.session_id} is already processing"
                )
        ...
        return ProcessSessionResponse(success=True, status="queued", session_id=request.session_id)
...
async def cancel_session(session_id: str):
    ...
    if result.status in [ProcessingStatus.COMPLETE, ProcessingStatus.FAILED]:
        raise HTTPException(status_code=400, detail=f"Session already {result.status.value}")

    # Mark as failed (cancellation)
    processing_tasks[session_id] = ProcessingResult(
        success=False, status=ProcessingStatus.FAILED, error_message="Cancelled by user"
    )
...
def auto_trigger_processing(
    ...
    **Integration Example (in views/sequence.py):**
    ...
    try:
        ...
        response = httpx.post(f"{backend_url}/api/processing/sessions", json=payload, timeout=10.0)
...
async def _process_session_task(
    session_id: str, fits_files: List[Path], target_name: str, metadata: Optional[Dict]
):
    """
    ...
    """
    # Mark as pending
    processing_tasks[session_id] = ProcessingResult(success=False, status=ProcessingStatus.PENDING)
...
async def health_check():
    ...
    try:
        ...
        result = subprocess.run(
            [siril_service.siril_bin, "--version"], capture_output=True, text=True, timeout=5
        )
