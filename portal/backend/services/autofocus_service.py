...
class AutoFocusService:
    ...
    async def _capture_frame(self) -> np.ndarray:
        ...
        """
        # Start exposure
        self.alpaca.start_exposure(
            duration=self.config.exposure_time,
            light=True
        )
    ...
    def _fit_v_curve(
        ...
        try:
            ...
            logger.info(f"V-curve fit: a={a:.6f}, b={b:.4f}, c={c:.2f}, R²={fit_params['r_squared']:.3f}")
