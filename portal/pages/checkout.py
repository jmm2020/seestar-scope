...
def _start_checkout(user_id: str, email: str) -> None:
    customer_id = _stripe_client.get_or_create_customer(user_id, email)
    if not customer_id:
    ...
    url = _stripe_client.create_checkout_session(
        user_id=user_id,
...
def _open_portal(user_id: str, email: str) -> None:
    ...
    url = _stripe_client.create_customer_portal_session(customer_id, return_url)
    if url:
