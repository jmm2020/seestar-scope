...
from backend.routers.processing import auto_trigger_processing
from backend.utils.image_processing_enhanced import save_dual_format
...
def _render_run_controls(alpaca):
    ...
    # Auto-processing option (Decision Q4: Auto-trigger with opt-out)
    st.checkbox(
        "Auto-process captured sequences with Siril",
        value=True,
        key="auto_process_enabled",
        help="Automatically stack frames with Siril when capture completes. "
        "Disable if you prefer to manually select frames for processing. "
        "NOTE: Requires sequence mode to auto-save FITS files (feature pending).",
    )
...
def _execute_sequence_step(alpaca):
    ...
    if idx >= len(targets):
        ...
        if st.session_state.get("auto_process_enabled", True):
            captured_files = st.session_state.get("captured_fits_files", [])

            if captured_files and targets:
                result = auto_trigger_processing(
                    session_id=f"{targets[-1]['name']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    fits_files=captured_files,
                    target_name=targets[-1]["name"],
                    metadata={"frames": len(captured_files)},
                )

                if result.get("success"):
                    st.info(f"Auto-processing queued: {result['session_id']}")
                else:
                    st.warning(f"Auto-processing failed: {result.get('error', 'unknown')}")

        st.session_state.captured_fits_files = []
    ...
    if step == "slew":
        ...
        if alpaca.is_image_ready():
            ...
            if image_data is None:
                st.warning("Camera readout failed — skipping frame")
                st.session_state["seq_step"] = "expose"
                st.rerun()
                return
            ...
            arr_min = arr.min()
            arr_norm = ((arr - arr_min) / (arr.max() - arr_min + 1e-6) * 255).astype(np.uint8)
            image_pil = Image.fromarray(arr_norm)
            ...
            save_result = save_dual_format(arr, image_pil, metadata, target["name"])

            if "captured_fits_files" not in st.session_state:
                st.session_state.captured_fits_files = []
            st.session_state.captured_fits_files.append(Path(save_result["fits"]))
